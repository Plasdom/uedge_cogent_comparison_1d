...
#                    (upim1 + upi)[:,1:,ii]\
        ...
        self.pmloss = zeros((com.nx+2,com.ny+2))
        ...
        if not bbb.isupgon[0]:
            ...
            self.pmomv = zeros((com.nx+2,com.ny+2))
        ...
        self.prdiss += bbb.ismolcrm*bbb.cmesore*bbb.edisse
        self.pibirth += (1-bbb.ismolcrm) * ( \
            bbb.ceisor*bbb.eion*bbb.ev*zeros((com.nx+2,com.ny+2))\
            - bbb.ccoldsor*bbb.ng[:,:,0]*( \
                1.5*bbb.ti - bbb.eion*bbb.ev \
            )*bbb.nucx[ix,iy,0]*com.vol) + bbb.ismolcrm*( \
                bbb.ceisor*bbb.cmesore*(  \
                    zeros((com.nx+2,com.ny+2)) + zeros((com.nx+2,com.ny+2))
                ) - bbb.ccoldsor*bbb.ng[:,:,0]*(
                    1.5*bbb.ti - bbb.eion*bbb.ev
                )*bbb.nucx[:,:,0]*com.vol
            )
    ...
    def pradpltwl(self):
        ...
        nj = com.nxomit 
#        if bbb.isimpon > 2:
        prdu = deepcopy(zeros((com.nx+2,com.ny+2)))
#        else:
